...
/*     getInput(): If we are not in chat mode, we want to move the camera via the keys,
so in this case assemble a translation vector that is then applied to the camera in moveCamera() 
...
// buildGUI(): Inserts all needed GUI elemnts in the SDKTrayManager 
...
// itemSelected(): CAllback function triggered when a selectionm menu changes. As a result when change the polygon mode of the camera 
