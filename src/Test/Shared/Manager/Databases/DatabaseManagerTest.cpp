...
BOOST_AUTO_TEST_CASE(testConnection)
{
    ...
    BOOST_CHECK_NO_THROW(databaseManager->connection(PARAMETRE));
...
BOOST_AUTO_TEST_CASE(testNonConnection)
{
    DatabaseManager * databaseManager = DatabaseManager::getInstance();
    databaseManager->createServer(POSTGRESQL);
    BOOST_CHECK_NO_THROW(databaseManager->connection("127.0.0.1;5432;Xsilium;Xsilium;xsilium_develop"));
    databaseManager->deconnection();
    DatabaseManager::DestroyInstance();
}
...
BOOST_AUTO_TEST_CASE(testDeconnection)
{
    ...
    BOOST_CHECK_NO_THROW(databaseManager->deconnection());
    DatabaseManager::DestroyInstance();
}
...
BOOST_AUTO_TEST_CASE(testSelectCommit)
{
    ...
    BOOST_CHECK_NO_THROW(databaseManager->executionPrepareStatement("test1",&resultat,commit));
