...
// requete qui va chercher les infos sur l'ip dans la table ip_banned
	connexionDatabase::PrepareStatement(REALMS_SEL_IPBANNED_INFOSSURIPBANNIES, "SELECT * FROM compte.ip_banned WHERE \"ip\" = $1");


//requete d'autoban apres erreur d'authentification -- Nico le 13-11-2012 modifiee le 15-11-2012 pour erreur de variables
connexionDatabase::PrepareStatement(REALMS_INS_ACCOUNTBANNED_AUTOBANCOMPTEAUTH, "INSERT INTO compte.account_banned VALUES (DEFAULT, now(), now() + INTERVAL  '20 minute', 'AutoBan erreur authentification', true, $1, $2)");
...
//Requete de mise a jour du nombre d'erreurs d'authentification -- nico - le 13-11-2012
	connexionDatabase::PrepareStatement(REALMS_UPD_ACCOUNT_MAJERREURSAUTH, "UPDATE compte.account SET \"failed_logins\" = $2 WHERE \"util_numero\" = $1");
...
//requete qui donne la date de fin de bannissement d'un compte donc le ban est actif --nico - le 13-11-2012
	connexionDatabase::PrepareStatement(REALMS_SEL_ACCOUNTBANNED_RECUPDATEFINBANCOMPTE, "SELECT \"unbandate\" FROM compte.account_banned WHERE \"active\" = true AND \"bandate\" <=now() AND \"id_user_ban\" = $1");
...
//requete qui recupere les infos en fonction du username --nico le 13-11-2012
	 connexionDatabase::PrepareStatement(REALMS_SEL_ACCOUNT_RECUPINFOSCOMPTE, "SELECT \"sha_pass_hash\",\"util_numero\",\"locked\",\"last_ip\",\"failed_logins\" FROM compte.account WHERE \"username\" = $1");
...
connexionDatabase::PrepareStatement(REALMS_UPD_ACCOUNTBANNED_DEBANCOMPTE, "UPDATE compte.account_banned SET \"active\" = false WHERE \"id_user_ban\" = $1");

//requete listing des infos de la liste des serveurs --Nico le 15-11-2012
connexionDatabase::PrepareStatement(REALMS_SEL_LISTESERVEUR_RECUPLISTESERVEUR, "SELECT \"id_serveur\", \"serveur_nom_serveur\", \"serveur_domain_name\", \"serveur_port\", \"serveur_level_requis\", \"serveur_n_online\", \"serveur_version_client_mini\", \"serveur_serveur_online\" FROM compte.liste_serveur");
...
connexionDatabase::PrepareStatement(REALMS_INS_BANIP_BANAUTOIP,"INSERT INTO compte.ip_banned VALUES ($1, now(), now() + INTERVAL  '20 minute', 'AutoBan pour erreur authentification ', DEFAULT, $2, true)");
...
connexionDatabase::PrepareStatement(REALMS_INS_IPTEMPORAIRE_STOCKAGEIPTEMPORAIRE,"INSERT INTO compte.ip_temporaire VALUES (DEFAULT, $1, '1')");
...
connexionDatabase::PrepareStatement(REALMS_DEL_IPTEMPORAIRE_SUPPRLIGNEIP,"DELETE FROM compte.ip_temporaire where \"ip_temp_nessais\" = '0'");
