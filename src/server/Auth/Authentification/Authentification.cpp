...
bool Authentification::FindClient(ENetAddress address)
{
	...
	for (client=listOfClient.begin() ; client!=listOfClient.end() ; ++client)
	{
	  if ( client->IP == address.host )
	  {
		  ...
		  {
			  log->Write(Log::INFO,"Le client avec l'IP : %d, Port : %d a ete trouvŽ ",client->IP,client->port);
			  return true;
		  }
	  }
	}
	log->Write(Log::INFO,"Le client avec l'IP : %d, Port : %d n'a pas ŽtŽ trouvŽ ",client->IP,client->port);
	return false;
}
