...
CompteBanDAO::CompteBanDAO() {
	...
	database->prepareStatement(suffix + database->ToString(REALMS_SEL_ACCOUNTBANNED),
			requeteSQL.c_str());
	...
	database->prepareStatement(suffix + database->ToString(REALMS_INS_ACCOUNTBANNED),
			requeteSQL.c_str());
	...
	database->prepareStatement(suffix + database->ToString(REALMS_UPD_ACCOUNTBANNED),
			requeteSQL.c_str());

	requeteSQL = "DELETE FROM compte.account_banned WHERE id_account_banned=$1";
	database->prepareStatement(suffix + database->ToString(REALMS_DEL_ACCOUNTBANNED),
			requeteSQL.c_str());
...
bool CompteBanDAO::create(CompteBan * compteBan, int idTransaction) {
	...
	retour = database->executionPrepareStatement(
			suffix + database->ToString(REALMS_INS_ACCOUNTBANNED), &tokens, idTransaction, 5,
			database->ToString(compteBan->getIdAccount()).c_str(),
			database->ToString(compteBan->getBandate()).c_str(),
			database->ToString(compteBan->getUnbandate()).c_str(), compteBan->getRaison().c_str(),
...
bool CompteBanDAO::update(CompteBan * compteBan, int idTransaction) {
	...
	return database->executionPrepareStatement(
			suffix + database->ToString(REALMS_UPD_ACCOUNTBANNED), &resultsqlT, idTransaction, 5,
			database->ToString(compteBan->getBandate()).c_str(),
			database->ToString(compteBan->getUnbandate()).c_str(), compteBan->getRaison().c_str(),
			database->ToString(compteBan->getBannedby()).c_str(),
...
bool CompteBanDAO::read(CompteBan * compteBan, int idTransaction) {
	...
	Tokens resultsqlT;
	retour = database->executionPrepareStatement(
			suffix + database->ToString(REALMS_SEL_ACCOUNTBANNED), &resultsqlT, idTransaction, 1,
			database->ToString(compteBan->getIdAccount()).c_str());
...
bool CompteBanDAO::suppr(CompteBan * compteBan, int idTransaction) {
	Tokens resultsqlT;
	return database->executionPrepareStatement(
			suffix + database->ToString(REALMS_DEL_ACCOUNTBANNED), &resultsqlT, idTransaction, 1,
			database->ToString(compteBan->getIdAccountBanned()).c_str());
}
