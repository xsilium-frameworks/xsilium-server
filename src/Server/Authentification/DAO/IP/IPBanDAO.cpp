...
bool IPBanDAO::create(Model * model, int idTransaction) {
    ...
    try {
        database->executionPrepareStatement(suffix + Utilities::toString(REALMS_INS_IPBANNED_BANIP),
                &tokens, idTransaction, 5, ipBan->getIp().c_str(),
...
bool IPBanDAO::update(Model * model, int idTransaction) {
    ...
    try {
        database->executionPrepareStatement(
                suffix + Utilities::toString(REALMS_UPD_IPBANNED_DEBANIP), &resultsqlT,
                idTransaction, 5, Utilities::toString(ipBan->getBandate()).c_str(),
                Utilities::toString(ipBan->getUnbandate()).c_str(), ipBan->getRaison().c_str(),
                Utilities::toString(ipBan->getBannedby()).c_str(),
